...
function RatingDot({ checked }) {
  return (
    <span
      className={[
        'inline-flex h-3.5 w-3.5 sm:h-4 sm:w-4 items-center justify-center rounded-full border',
        checked ? 'border-white bg-white' : 'border-gray-300 bg-gray-200',
      ].join(' ')}
    />
  );
}
                  ...
                  {[5, 4, 3, 2, 1].map((n) => (
                    <div key={n} className="flex justify-center">
                      <label className="cursor-pointer">
                        ...
                        />
                        <RatingDot checked={ratings[c.key] === n} />
                      </label>
                    </div>
                  ))}
                </div>
